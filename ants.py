    ...
    def nearest_bee(self):
        ...
        while location.entrance is not None:
            ...
            if not location.is_hive:
                if len(location.bees) > 0 and steps >= self.min_range:
...
class WallAnt(Ant):
    """
    We are going to add some protection to our glorious home base by implementing the WallAnt, an ant that does nothing each turn.
    """
    name = 'Wall'
    damage = 0
    food_cost = 4
...
class TankAnt(ContainerAnt):
    """
    The TankAnt is a container ant that protects an ant in its place and also deals 1 damage to all bees in its place each turn.
    """
    name = 'Tank'
    damage = 1
    food_cost = 6
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        while position:
            if position.ant is not None:
                if not position.ant.buffed:
                    position.ant.buffed = True
                    position.ant.buff()
                if position.ant.is_container and position.ant.ant_contained is not None:
                    if not position.ant.ant_contained.buffed:
                        position.ant.ant_contained.buff()
                        position.ant.ant_contained.buffed = True
    ...
    def reduce_health(self, amount):
        ...
        remaining, sigal the end of the game.
        """
        # BEGIN Problem 12
        if self.health <= amount:
            ants_lose()
